...
class Runtime(module.Module):
    ...
    def __init__(self, hidden_size, num_layers, output_shapes, num_heads, hidden_sizes):
    ...
    def build_program(self, seed):
        ...
        return Q_ws, K_ws, V_ws, S_ws, _ws, H_bs, F_ws, F_bs, G_w, G_b

    def execute_program(self, seed, environment, all_probs):
        ...
        left_result, left_probs = self.execute_program(left, environment, all_probs)
        right_result, right_probs = self.execute_program(right, environment, all_probs)
        ...
        padded_right_probs = tf.pad(right_probs, [[0, 0], [0, tf.shape(residual_left_probs)[1]]])
        ...
        joined_result, joined_probs = self.execute_program(left_result,
            tf.concat([tf.expand_dims(right_result, 1), environment], 1), joined_probs)
        # Merge the results according to the mask
        next_seed = tf.where(mask, right, joined_result)
        next_probs = tf.where(mask, padded_right_probs, joined_probs)
        return next_seed, next_probs

    def __call__(self, seed, environment):
        return self.execute_program(seed, environment)
